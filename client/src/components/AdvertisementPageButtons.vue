<template>
  <template v-if="userStore.user">
    <template v-if="ad?.User?.id == userStore.user.id">
      ...
      <button type="button" class="btn btn-outline-danger w-100" @click="removeAd()">
        Удалить объявление
      </button>
    </template>

    <template v-else-if="userStore.user.role == userRoles.MODER && ad.moderStatus == adModerStatus.CREATED">
      <button type="button" class="btn btn-primary w-100 mb-3" @click="publish()">
        Опубликовать объявление
      </button>

      <button type="button" class="btn btn-outline-danger w-100" @click="refuse()">
    ...
    <template v-else-if="userStore.user.role == userRoles.USER && ad?.User?.id != userStore.user.id">
      ...
      <div v-else-if="ad?.expertStatus == adExpertStatus.REQUESTED" class="card text-white bg-success">
        <div class="card-body">
  ...
  <div v-else class="card">
    <div class="card-body">
      Зарегистрируйтесь или войдите в систему, чтобы написать сообщение продавцу или запросить экспертную оценку объявления.
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async setReview() {
      if (confirm("Вы действительно хотите оставить экспертную оценку?")) {
        ...
        await this.advertisementsStore.editExpert(this.ad.id, adExpertStatus.REVIEWED, review);
        this.$router.go();
      }
    },
    async publish() {
      if (confirm("Вы действительно хотите опубликовать объявление?")) {
        await this.advertisementsStore.editModer(this.ad.id, adModerStatus.PUBLISHED);
      }
    },
    async refuse() {
      if (confirm("Вы действительно хотите отказать в публикации?")) {
        let reason = prompt("Введите причину: ");
        await this.advertisementsStore.editModer(this.ad.id, adModerStatus.REFUSED, reason);
      }
    },
    async removeAd() {
